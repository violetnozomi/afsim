...
protected:
   void SetUp() override
   {
      ...
      const std::string ACO = mDataService.getResource("Aco.txt").GetNormalizedPath();

      usmtf::USMTF_Parser parser = usmtf::USMTF_Parser(ACO);
...
TEST_F(AcoTest, AcoIsValidForValidSets)
{
   ASSERT_TRUE(mMessage->IsValid());
}

TEST_F(AcoTest, AcoExportWorks)
{
   EXPECT_TRUE(mMessage->IsValid());

   const ut::TestDataService& dataService     = ut::Locator<ut::TestDataService>::get();
   const std::string          ACO_EXPORT      = dataService.getResource("AcoExport.txt").GetNormalizedPath();
   const std::string          ACO_EXPORT_TEMP = dataService.getResource("AcoExport_Temp.txt").GetNormalizedPath();
