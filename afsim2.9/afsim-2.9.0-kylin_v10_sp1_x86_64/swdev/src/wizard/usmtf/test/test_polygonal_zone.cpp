...
class PolygonZoneTest : public ::testing::Test
{
protected:
   void SetUp() override
   {
      ...
      const std::string CZACO = mDataService.getResource("PolygonZoneAco.txt").GetNormalizedPath();
...
TEST_F(PolygonZoneTest, PolygonZoneAcoExportIsValidForValidSets)
{
   ASSERT_TRUE(mMessage->IsValid());
}

TEST_F(PolygonZoneTest, PolygonZoneAcoExportWorks)
{
   EXPECT_TRUE(mMessage->IsValid());

   const ut::TestDataService& dataService = ut::Locator<ut::TestDataService>::get();
   const std::string          ACO_EXPORT  = dataService.getResource("PolygonZoneAcoExport.txt").GetNormalizedPath();
   const std::string   ACO_EXPORT_TEMP = dataService.getResource("PolygonZoneAcoExport_Temp.txt").GetNormalizedPath();
