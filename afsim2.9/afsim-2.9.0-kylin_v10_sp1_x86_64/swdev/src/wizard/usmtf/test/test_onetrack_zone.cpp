...
class OneTrackZoneTest : public ::testing::Test
{
protected:
   void SetUp() override
   {
      ...
      const std::string CZACO = mDataService.getResource("OnetrackZoneAco.txt").GetNormalizedPath();
...
TEST_F(OneTrackZoneTest, OneTrackZoneTestAcoExportIsValidForValidSets)
{
   ASSERT_TRUE(mMessage->IsValid());
}

TEST_F(OneTrackZoneTest, OneTrackZoneTestZoneAcoExportWorks)
{
   EXPECT_TRUE(mMessage->IsValid());

   const ut::TestDataService& dataService = ut::Locator<ut::TestDataService>::get();
   const std::string          ACO_EXPORT  = dataService.getResource("OneTrackZoneAcoExport.txt").GetNormalizedPath();
   const std::string   ACO_EXPORT_TEMP = dataService.getResource("OneTrackZoneAcoExport_Temp.txt").GetNormalizedPath();
