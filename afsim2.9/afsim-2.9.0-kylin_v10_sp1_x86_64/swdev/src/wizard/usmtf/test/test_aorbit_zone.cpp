...
class AOrbitZoneTest : public ::testing::Test
{
protected:
   void SetUp() override
   {
      ...
      const std::string azACO = mDataService.getResource("AOrbitZoneAco.txt").GetNormalizedPath();

      usmtf::USMTF_Parser parser = usmtf::USMTF_Parser(azACO);
...
TEST_F(AOrbitZoneTest, AorbitZoneAcoExportIsValidForValidSets)
{
   ASSERT_TRUE(mMessage->IsValid());
}

TEST_F(AOrbitZoneTest, AorbitZoneAcoExportExportWorks)
{
   EXPECT_TRUE(mMessage->IsValid());

   const ut::TestDataService& dataService = ut::Locator<ut::TestDataService>::get();
   const std::string          ACO_EXPORT  = dataService.getResource("AOrbitZoneAcoExport.txt").GetNormalizedPath();
   const std::string   ACO_EXPORT_TEMP    = dataService.getResource("AOrbitZoneAcoExport_Temp.txt").GetNormalizedPath();
