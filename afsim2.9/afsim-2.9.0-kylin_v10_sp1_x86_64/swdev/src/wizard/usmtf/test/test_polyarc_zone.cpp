...
class PolyarcZoneTest : public ::testing::Test
{
protected:
   void SetUp() override
   {
      ...
      const std::string CZACO = mDataService.getResource("PolyarcZoneAco.txt").GetNormalizedPath();
...
TEST_F(PolyarcZoneTest, PolyarcZoneAcoExportIsValidForValidSets)
{
   ASSERT_TRUE(mMessage->IsValid());
}

TEST_F(PolyarcZoneTest, PolyarcZoneZoneAcoExportWorks)
{
   EXPECT_TRUE(mMessage->IsValid());

   const ut::TestDataService& dataService = ut::Locator<ut::TestDataService>::get();
   const std::string          ACO_EXPORT  = dataService.getResource("PolyarcZoneAcoExport.txt").GetNormalizedPath();
   const std::string   ACO_EXPORT_TEMP = dataService.getResource("PolyarcZoneAcoExport_Temp.txt").GetNormalizedPath();
