...
// ============================================================================
LayerImp* PresentationLayer::Clone() const
{
   return new PresentationLayer(*this);
}
...
// virtual
bool PresentationLayer::Receive(double aSimTime, Comm* aXmtrPtr, Message& aMessage, bool& aNotifyNextLayer)
{
   bool notify  = true;
   bool success = true;
   bool halt    = false;
   ...
   {
      auto result = protocol->PresentationReceive(aSimTime, GetIndex(), aMessage);
      halt        = result.first;
...
//! @return 'true' if layer successfully processed the message.
// virtual
bool PresentationLayer::Send(double aSimTime, size_t aLayerIndex, Message& aMessage, bool& aNotifyNextLayer)
{
   bool notify  = true;
   bool success = true;
   bool halt    = false;
   ...
   for (auto protocol : protocols)
   {
      auto result = protocol->PresentationSend(aSimTime, GetIndex(), aMessage);
      halt        = result.first;
      notify &= result.second;
      if (halt)
      {
...
// virtual
bool PresentationLayer::ProcessLayerMessage(double aSimTime, layer::Message aLayerMessage, Message* aCommMessagePtr)
{
   bool success = true;
