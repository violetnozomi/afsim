...
WSF_CYBER_TOGGLE_WEAPONS_EFFECT
-------------------------------

.. model:: cyber_effect WSF_CYBER_TOGGLE_WEAPONS_EFFECT

.. parsed-literal::

   cyber_effect <effect_name> :model:`WSF_CYBER_TOGGLE_WEAPONS_EFFECT`
    
      platform_type <type>
         ... weapon_effect_targeting_ definitions
...
:model:`WSF_CYBER_TOGGLE_WEAPONS_EFFECT` is a cyber_effect that allows disabling of one
or more weapons on affected victim platforms.
...
.. _weapon_effect_targeting:

Weapon Effect Targeting
^^^^^^^^^^^^^^^^^^^^^^^

For a platform subject to this effect and matching a defined platform, platform_type, or default block,
the following options are provided for determining which weapons on the victim are subject to being toggled off:

.. command:: target_list <weapon-name> ... <weapon-name> ... end_target_list

   A target_list defines the names of the weapons that should be disabled by this effect. Any weapon name
   matching the provided names will be turned off. All other weapons will be unaffected. 
   
.. command:: select_all

   Specifying this command in lieu of the target_list will disable all weapons on the victim platform. Defining
