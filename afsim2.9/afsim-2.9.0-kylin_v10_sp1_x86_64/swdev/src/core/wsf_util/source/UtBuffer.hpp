...
//! A simple buffer with operations to read/write to memory.
//! Note: Put methods will grow the buffer if the 'grow' option is set.
//! Otherwise, all Put and Get methods require the user to know that
//! there is enough space to do the operation!
class WSF_UT_EXPORT UtBuffer
{
public:
   ...
   UtBuffer(const UtBuffer& aSrc)
      : mBuffer(new char[aSrc.mBytes])
      , mBytes(aSrc.mBytes)
      , mGetPos(aSrc.mGetPos)
      , mPutPos()
      , mCanGrow(true)
      , mSwapBytes(aSrc.mSwapBytes)
   {
      if (aSrc.mPutPos > mGetPos)
      {
   ...
   //! @param aCanGrow If 'true', UtBuffer takes ownership of the buffer and allows resizing
   //!                 If 'false', UtBuffer will not delete the buffer or resize.
   ...
   //! Put a basic type into the buffer.  This should only be used for basic types (char, int, float).
   ...
   //! Get a basic type from the buffer.  This should only be used for basic types (char, int, float)
   //! This will perform a byte-swap if that option is enabled
   template<typename T>
   void Get(T& aVal)
   {
   ...
   {
      bool needsToGrow = (aPutSize + mPutPos >= mBytes);
      if (mCanGrow & needsToGrow)
      {
         GrowBy(aPutSize);
      }
      return needsToGrow & !mCanGrow;
   }
