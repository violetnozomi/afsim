...
if '-x' not in sys.argv:
    if os.name == 'nt':
        exes += glob.glob('../../*BUILD*/Release/sensor_plot*.exe') 
        exes += glob.glob('../../*BUILD*/Debug/sensor_plot*.exe')
        exes += glob.glob('../../../*BUILD*/Release/sensor_plot*.exe')
        exes += glob.glob('../../../*BUILD*/Debug/sensor_plot*.exe')
        exes += glob.glob('../../*BUILD*/Release/sensor_plot*.exe')
        exes += glob.glob('../../*BUILD*/Debug/sensor_plot*.exe')
        ...
        exes += glob.glob('../../*BUILD*/sensor_plot_' + sysType + model + '.exe')
        exes += glob.glob('../../../*BUILD*/sensor_plot_' + sysType + model + '.exe')
        exes += glob.glob('../../*BUILD*/sensor_plot_' + sysType + model + '.exe')
...
#   2) The core included project specific 'tests' directories (e.g.: ../../wsf_project/test_sensor_plot)
#   3) The extension project specific 'tests' directories (e.g.: ../../../wsf_project/test_sensor_plot)
if '-d' not in sys.argv:
    dirs += ['-d', 'auto_*']
    dirs += ['-d', '../../*/test_sensor_plot']
    dirs += ['-d', '../../*/*/test_sensor_plot']
