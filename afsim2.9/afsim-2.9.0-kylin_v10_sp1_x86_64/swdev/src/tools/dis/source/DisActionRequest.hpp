...
#ifndef DISACTIONREQUEST_HPP
#define DISACTIONREQUEST_HPP
...
class DIS_EXPORT DisActionRequest : public DisPdu
{
public:
   DisActionRequest();
   DisActionRequest(DisPdu& aPdu, GenI& aGenI);
   DisActionRequest(GenI& aGenI);
   DisActionRequest(const DisActionRequest& aSrc);

   ~DisActionRequest() override;
...
private:
   // void Copy(const DisActionRequest& aRhs);
...
inline DisUint32 DisActionRequest::GetActionId() const
{
   return mActionId;
}
...
inline void DisActionRequest::SetActionId(DisEnum32 aActionId)
{
   mActionId = aActionId;
}

inline void DisActionRequest::SetDatumSpec(const DisDatumSpec& aDatumSpec)
{
   mData = aDatumSpec;
}
