...
Reference material (not used in generating the compiler):
---------------------------------------------------------
...
1) Set your working directory to this directory (util_script/source/script_parser).
...
The resulting output files are modified by 'update_parser.py' and written to:

   Parser.h     -> ../UtScriptLanguageParser.hpp
   Parser.cpp   -> ../UtScriptLanguageParser.cpp
   Scanner.h    -> ../UtScriptLanguageScanner.hpp
   Scanner.cpp  -> ../UtScriptLanguageScanner.cpp

These files, along with any modified .frame file or script.atg file, should be commited.

Wizard
======
Changes to these Scanner/Parser requires corresponding changes to the Wizard Scanner/Parser files.
See wizard/lib/source/Us/AAA_README.TXT.

Testing
=======

The standard WSF auto_tests should be run prior to committing any updates. The test cases
in auto_test/auto_script will test a number of language constructs to ensure valid constructs
are accepted and invalid constructs are rejected.
