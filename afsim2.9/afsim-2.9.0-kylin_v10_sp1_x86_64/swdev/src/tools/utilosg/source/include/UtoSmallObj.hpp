...
#include "../Utility/UtoSingleton.hpp"
#include "../Utility/UtoThreads.hpp"
...
public:
   ...
   bool operator<(std::size_t rhs) const { return BlockSize() < rhs; }
   bool operator<(const FixedAllocator& rhs) const { return BlockSize() < rhs.BlockSize(); }
};
...
public:
   static void* operator new(std::size_t size)
   {
      ...
      typename MyThreadingModel::Lock lock(s_ThreadingModelData);
...
private:
   static MyThreadingModel s_ThreadingModelData;
};

// static member definitions
template<template<class> class ThreadingModel, std::size_t chunkSize, std::size_t maxSmallObjectSize>
ThreadingModel<SmallObject<ThreadingModel, chunkSize, maxSmallObjectSize>>
   SmallObject<ThreadingModel, chunkSize, maxSmallObjectSize>::s_ThreadingModelData;
