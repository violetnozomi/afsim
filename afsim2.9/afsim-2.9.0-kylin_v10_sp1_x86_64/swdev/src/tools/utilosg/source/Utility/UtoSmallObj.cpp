   ...
   , chunks_(rhs.chunks_)
{
   ...
   allocChunk_ = rhs.allocChunk_ ? &chunks_.front() + (rhs.allocChunk_ - &rhs.chunks_.front()) : nullptr;

   deallocChunk_ = rhs.deallocChunk_ ? &chunks_.front() + (rhs.deallocChunk_ - &rhs.chunks_.front()) : nullptr;
}
...
FixedAllocator::~FixedAllocator()
{
   ...
   for (; i != chunks_.end(); ++i)
   {
      // CME_MOD - Temporarily comment this out until we figure
      // out why this condition isn't valid.
...
/* The STL headers shipped  Visual Studio 8.0 (_MSC_VER>=1400) will check the
 * less than comparison lhs<rhs, with rhs<lhs which requires a comparison
 * operator with the types switched.
 */
#if defined(_DEBUG) && (_MSC_VER >= 1400)
namespace std
{
bool operator<(std::size_t lhs, const Loki::FixedAllocator& rhs)
{
   return lhs < rhs.BlockSize();
}
} // namespace std
#endif
