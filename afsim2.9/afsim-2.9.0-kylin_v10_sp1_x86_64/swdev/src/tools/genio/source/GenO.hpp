...
/// \file  GenO.hpp
/// \brief GenO
/// \class GenO GenO.hpp "GenO.hpp"
/// \brief This base class defines the interface for the output side of GenIO.
///
...
/// sends data then it should inherit directly from this class.  If the transport class
/// sends and receives data then it should inherit from the GenIO class which inherits from
/// this class.
///
/// This class uses the concept that messages or data is "Put" to the send buffer for bundling and then sent at once
/// with the Send() method.  If the send buffer is full, the buffered data will automatically be sent before accepting
/// additional data from any of the Put methods.  Data is not guaranteed to be sent until the Send() method is called.
/// A call to Send() with an empty send buffer is harmless.\n
///
/// The GenOHelper class can be used when sending messages to correctly manage the send buffer.
...
public:
   ...
   /// Put a vector of a built in type in the send buffer.
   ...
   /// Put an array of built in types into the send buffer.
   /// \return A negative number if there is an error, else returns the number of bytes put in the send buffer.
   template<class T>
   ...
   /// Put method for raw data.  No byte swapping is performed.  This function is not used by most users.
   ...
   /// Put method for raw data.  No byte swapping is performed.  No data is physically
   ...
   /// Set the Put offset.  This function is not normally used, but can be handy if the
   ...
   /// Get the Put offset.  This function is not normally used, but can be handy if the
...
protected:
   ...
   // Give the derived class a chance to handle an error in a Put() method.
   // \return Returns a positive number if the error was handled so another Put
