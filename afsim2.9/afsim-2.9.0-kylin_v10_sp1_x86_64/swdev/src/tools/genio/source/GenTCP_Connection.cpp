...
GenTCP_Connection::GenTCP_Connection()
   : GenSocketConnection(new GenSockets::GenSocket(GenSockets::GenSocket::cTCP_SOCKET))
   , mUseMessageHeaders(false)
...
bool GenTCP_Connection::Init(const std::string& aSendAddress, int aSendToPort, float aWaitTimeSeconds)
{
   ...
   if (inAddr.IsValid())
   {
      mSendAddress     = ut::make_unique<GenSockets::GenInternetSocketAddress>(inAddr, aSendToPort);
   ...
   {
      auto out = ut::log::error() << "Error: Unknown host.";
      out.AddNote() << "Host Address: " << aSendAddress;
   }
   return mIsInitialized;
}
...
void GenTCP_Connection::ResetSocket(std::unique_ptr<GenSockets::GenSocket> aNewSocket)
{
   ...
   {
      mSocket = ut::make_unique<GenSockets::GenSocket>(GenSockets::GenSocket::cTCP_SOCKET);
   }
}
