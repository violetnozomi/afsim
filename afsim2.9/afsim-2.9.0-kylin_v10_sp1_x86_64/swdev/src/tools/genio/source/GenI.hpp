...
public:
   ...
   ///         for providing enough memory for the Get method to transfer all the characters including the
   ///         null zero.
   ...
   /// Get a vector of a built in type from the receive buffer.
   ...
   /// Get an array of built in types from the receive buffer.
   /// \return A negative number if there is an error, else returns the number of
   ///         bytes gotten from the receive buffer..
   template<class T>
   ...
   /// Get a string from the receive buffer.
   ...
   /// Get method for raw data.  No byte swapping is performed.  This function is not used by most users.
   ...
   /// Set the Get offset.  This function is not normally used, but can be handy if the
   ...
   /// Get the Get offset.  This function is not normally used, but can be handy if the
...
protected:
   ...
   // Give the derived class a chance to handle an error in a Get() method.
   // \return Returns a positive number if the error was handled so another Get
