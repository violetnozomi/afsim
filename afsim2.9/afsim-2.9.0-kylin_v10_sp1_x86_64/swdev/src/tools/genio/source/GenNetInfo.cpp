...
GenNetInfo::GenNetInfo()
{
   ...
   if (sd == SOCKET_ERROR)
   {
      ut::log::error() << "Winsock: " << WSAGetLastError();
      return;
   }
   ...
   for (int i = 0; i < numInterfaces; ++i)
   {
      ...
      if (result == NULL)
      {
         auto out = ut::log::error() << "Failed to convert address to string.";
      ...
      if (result == NULL)
      {
         auto out = ut::log::error() << "Failed to convert broadcast address to string.";
         out.AddNote() << "Errno: " << std::strerror(errno);
         continue;
      }
      ...
      if (result == NULL)
      {
         auto out = ut::log::error() << "Failed to convert netmask to string.";
         out.AddNote() << "Errno: " << std::strerror(errno);
         continue;
      }
...
void GenNetInfo::Print()
{
   ...
   auto out = ut::log::info() << "GenNetInfo Data Dump:";

   for (int i = 0; i < numInterfaces; ++i)
   {
      Interface* interfacePtr = GetInstance().mInterfaces[i];
      auto       note         = out.AddNote() << "Interface " << i << ":" << std::boolalpha;

      note.AddNote() << "Address: " << interfacePtr->mInfoPtr->mAddr;
      note.AddNote() << "Broadcast Address: " << interfacePtr->mInfoPtr->mBroadcastAddr;
      note.AddNote() << "Netmask: " << interfacePtr->mInfoPtr->mNetmask;

      note.AddNote() << "Up: " << interfacePtr->mInfoPtr->mIsUp;
      note.AddNote() << "Point-to-Point: " << interfacePtr->mInfoPtr->mIsPointToPoint;
      note.AddNote() << "Loopback: " << interfacePtr->mInfoPtr->mIsLoopback;

      note.AddNote() << "Has Broadcast: " << interfacePtr->mInfoPtr->mHasBroadcast;
      note.AddNote() << "Has Multicast: " << interfacePtr->mInfoPtr->mHasMulticast;
   }
}
