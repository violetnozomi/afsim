   ...
   if (mErrorStatus == GenBuf::NoError)
   {
      ...
      if (aStatus > GenBuf::FixFailed)
      {
         if (GenError::ShowError(GenError::cIO_ERRORS))
         {
            ...
            auto out = ut::log::error() << "GenO called HandlePutError.";
            out.AddNote() << "Wall Clock: " << GenTime::GetWallClockTime();
            out.AddNote() << "GenO: " << mName;
            out.AddNote() << "Error: " << msg[-aStatus] << " when trying to read " << aVariableSize << " bytes.";
         }
      }
   }
