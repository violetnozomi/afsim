...
// GenBuf - A base class to hold a buffer for the GenBufXferI
//          and GenBufXferO classes.
...
public:
   ...
   // Put all pointers back to the beginning
   void Reset();
...
inline int GenBuf::EnsurePutSpace(unsigned long aVariableSize)
{
   ...
   {
      // Let Put buffer grow if we own the buffer
