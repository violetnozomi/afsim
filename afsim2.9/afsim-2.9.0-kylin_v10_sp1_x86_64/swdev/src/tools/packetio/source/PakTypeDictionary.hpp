...
#ifndef PAKTYPEDICTIONARY_HPP
#define PAKTYPEDICTIONARY_HPP
...
class UtNullCreate;
//
// PakTypeDictionary provides allows for polymorphic Serialize() methods
// Prior to serializing a type, PakTypeDictionary::RegisterType<TheType>() must be called
// When the type is serialized using PakSerialization::Polymorphic(...), the correct
// Serialize() method will be called for that type.  If the type wasn't registered, an assert
// will be thrown.
//
...
class PACKETIO_EXPORT PakTypeDictionary
{
public:
...
private:
   ...
   {
      Data()
         : mCreateFnPtr(nullptr)
         , mSerializeInFnPtr(nullptr)
         , mSerializeOutFnPtr(nullptr)
      {
      }
