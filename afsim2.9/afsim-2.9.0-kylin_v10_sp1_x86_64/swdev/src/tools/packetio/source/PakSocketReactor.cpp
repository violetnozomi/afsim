...
PakSocketReactor::PakSocketReactor()
   : mIsRunning(false)
   , mImpl(new PakSocketReactorImpl)
{
...
void PakSocketReactor::RunSelect(double aWaitTime, int aEventType)
{
   ...
   {
      if (GenSockets::GenSocketSelector::cERROR ==
          mImpl->mSocketSelector.Select(mImpl->mSelectedSockets, ((float)aWaitTime), aEventType))
      {
         if (!RemoveErrorSockets())
         {
            ut::log::error() << "Unknown error on GenSocketSelector::Select().";
         }
      }
   }
}
