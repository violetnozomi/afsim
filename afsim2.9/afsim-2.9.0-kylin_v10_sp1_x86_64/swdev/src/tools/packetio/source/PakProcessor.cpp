...
PakPacket* PakProcessor::ReadPacket(PakSocketIO& aIO)
{
   ...
   if (aIO.ReceiveHeader(id, length, 0))
   {
      ...
      if (pInfo == nullptr)
      {
         auto out = ut::log::error() << "Received unknown packet.";
         out.AddNote() << "ID: " << id;
         aIO.IgnorePacket();
      }
      else
...
void PakProcessor::SubscribeP(int aPacketId, UtCallback* aCallbackPtr, bool aIsSpecific)
{
   ...
   {
      { // RAII block
         auto out = ut::log::fatal() << "Packet is not registered.";
         out.AddNote() << "ID: " << aPacketId;
      }
      exit(0);
   }
}
                                                        ...
                                                        int                 aPacketBaseId)
{
   ...
   {
      { // RAII block
         auto out = ut::log::fatal() << "Packet already initialized, cannot RegisterPacket again.";
         out.AddNote() << "Name: " << aPacketName;
         out.AddNote() << "ID: " << aPacketId;
      }
      exit(0);
   }
...
PakProcessor::PacketInfo::PacketInfo(int aPacketId, std::string aPacketName, PacketCallbackList* aCallbackListPtr, bool aIsUndefined)
   ...
   , mSpecificCallbackList(aCallbackListPtr)
{
   mPacketID          = aPacketId;
   mPacketName        = aPacketName;
