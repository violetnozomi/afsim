...
/*   vec3 light = gl_LightSource[0].position.xyz; // this is the vector to the sun

   vec4 v4ZeroPos = osg_ViewMatrix * vec4(0, 0, 0, 1);
   vec3 v3CameraPos = -(v4ZeroPos.xyz);
   float fDepth = length(v3CameraPos);

   float z_b = texture2D(depth, gl_TexCoord[0].st).x;
   float z_n = 2.0 * z_b - 1.0;
   float z_e = 2.0 * UTO_DepthRange.x * UTO_DepthRange.y / (UTO_DepthRange.y + UTO_DepthRange.x - z_n * (UTO_DepthRange.y - UTO_DepthRange.x));

   vec4 ndcPos;
   ndcPos.xy = 2.0 * gl_TexCoord[0].st - 1.0;
...
// Initialize the scattering loop variables
   ...
   vec3 v3FrontColor = texColor.rgb;
   vec3 v3Attenuate;
   float fScatter;
...
// Finally, scale the Mie and Rayleigh colors and set up the varying variables for the pixel shader
   ...
   vec3 scaleColor = vec3(pow(sColor.r * 0.25, 0.5), pow(sColor.g * 0.25, 0.5), pow(sColor.b * 0.30, 0.5));
