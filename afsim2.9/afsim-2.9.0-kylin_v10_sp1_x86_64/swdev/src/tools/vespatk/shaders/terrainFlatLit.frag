...
varying vec2 geodetic;

void main( void )
{
   vec4 vColor = texture2D(Texture0, gl_TexCoord[0].xy);
   vec4 vColor7 = texture2D(Texture7, gl_TexCoord[7].xy);
   vec3 norm = vec3(cos(geodetic.x) * cos(geodetic.y), sin(geodetic.x) * cos(geodetic.y), sin(geodetic.y));
   float LightIntensity = gl_LightSource[0].ambient.r + 2 / (1 + exp(-6 * dot(gl_LightSource[0].position.xyz, norm))) - 1;
   float mean = 0.3333 * (vColor.r + vColor.g + vColor.b);
   vec4 vSatColor = vec4(fSaturation * (vColor.rgb - mean) + vColor.rgb, vColor.a);
   vec4 vBrightColor = vec4(LightIntensity * (vSatColor.rgb + (1.0 - vSatColor.rgb) * fBrightener + (-vSatColor.rgb) * fDarkener), vSatColor.a);
