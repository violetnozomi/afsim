...
vec4 GetModelZero()
{
   mat4 osg_ModelMatrix = osg_ViewMatrixInverse * gl_ModelViewMatrix;
...
vec4 ScreenSpaceTransform(vec4 aInput)
{
   ...
   return gl_ModelViewProjectionMatrix * vec4(aInput.xyz, scalar);
}
...
vec4 BillboardTransform(vec4 aInput)
{
   ...
   return gl_ProjectionMatrix * mm * vec4(aInput.xyz, scalar);
}
