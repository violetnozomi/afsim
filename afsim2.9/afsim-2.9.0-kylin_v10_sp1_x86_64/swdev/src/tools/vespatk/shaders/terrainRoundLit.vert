...
void main( void )
{
   ...
   vec4 vEarth = viewMatrix * vec4(0, 0, 0, 1); // this is the eye to database origin (earth's center) vector
   ...
   LightIntensity = min((4 * fSpaceFactor + 1) * max(dot(gl_LightSource[0].position.xyz, tnorm), 0.0) + gl_LightSource[0].ambient.x, 1.0);
