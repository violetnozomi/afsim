...
#ifndef SIXDOFDATAPLATFORMDATA_HPP
#define SIXDOFDATAPLATFORMDATA_HPP
...
public:
   ...
   void AddPlatform(const std::string& aPlatformName) { mPlatforms.insert(aPlatformName); }
   void RemovePlatform(const std::string& aPlatformName) { mPlatforms.erase(aPlatformName); }
   bool IsValidPlatform(const std::string& aPlatformName) const { return mPlatforms.count(aPlatformName) > 0; }
   void ClearPlatforms() { mPlatforms.clear(); }

private:
   std::string           mPlatformOfInterest;
   PlatformData          mData;
   std::set<std::string> mPlatforms;
};
