...
#include "WsfScriptUCI_ESM_SettingsCommandMessage.hpp"
...
UCI_ESM_SettingsCommandMessageClass::UCI_ESM_SettingsCommandMessageClass(const std::string& aClassName,
...
UT_DEFINE_SCRIPT_METHOD(UCI_ESM_SettingsCommandMessageClass, UCI_ESM_SettingsCommandMessage, Construct, 2, "UCI_ESM_SettingsCommandMessage", "UCI_CapabilityId, UCI_CapabilityState")
{
   auto interfacePtr = UCI_Util::GetInterface(SIMULATION);
   if (interfacePtr != nullptr)
   {
      auto messagePtr = new UCI_ESM_SettingsCommandMessage(*interfacePtr);
