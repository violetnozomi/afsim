// This file was generated  on 12/5/2018 at 1:1:47 PM by the Boeing OMS CAL generation tools
// @warning  This file was automatically generated, edit at your own risk

/**
* Unclassified               U N C L A S S I F I E D               Unclassified
*
* DEVELOPMENT:
*    This document wholly developed with USG funds.
*    For additional information, contact the AFRCO.
*
* ‒  DISTRIBUTION STATEMENT D.  Distribution authorized to the Department
*    of Defense and U.S. DoD contractors only; Critical Technology; 17 Sep 2015.
*    Other requests shall be referred to Air Force Rapid Capabilities Office,
*    Bolling AFB, Washington DC 20032-6400. 
*
* EXPORT CONTROL:
*    WARNING - ITAR CONTROLLED - US ONLY
*    This distribution contains technical data whose export is restricted by
*    the Arms Export Control Act (Title 22, U.S.C., Sec. 2751 et seq. or the
*    Export Administration Act of 1979 as amended Title 50, U.S.C., App.
*    2401-2420 et seq.), as amended. Violation of these export laws are subject
*    to severe criminal penalties.  Disseminate in accordance with provisions of DoD
*    Directive 5230.25.
*/
#ifndef Uci__Type__ProductProcessingTaskStatusMDT_h
#define Uci__Type__ProductProcessingTaskStatusMDT_h 1

#if !defined(Uci__Type__ProductManagementTaskStatusDataType_h)
# include "uci/type/ProductManagementTaskStatusDataType.h"
#endif

#if !defined(Uci__Type__ProductProcessingStatusType_h)
# include "uci/type/ProductProcessingStatusType.h"
#endif

//  The namespace in which all UAS C2 Initiative data types are declared
namespace uci {

   //  The namespace in which all generated data types are declared
   namespace type {

      /** This is the ProductProcessingTaskStatusMDT sequence accessor class */
      class ProductProcessingTaskStatusMDT : public virtual uci::type::ProductManagementTaskStatusDataType {
      public:

         /** The destructor */
         virtual ~ProductProcessingTaskStatusMDT()
         { }

         /** Returns this accessor's type constant, i.e. ProductProcessingTaskStatusMDT
           *
           * @return This accessor's type constant, i.e. ProductProcessingTaskStatusMDT
           */
         virtual uci::base::accessorType::AccessorType getAccessorType() const
            throw()
         {
            return uci::type::accessorType::productProcessingTaskStatusMDT;
         }


         /** Initializes the contents of this accessor by copying the contents of the specified accessor
           *
           * @param accessor The accessor whose contents are to be used to initialize the contents of this accessor
           */
         virtual void copy(const ProductProcessingTaskStatusMDT& accessor)
            throw(uci::base::UCIException) = 0;


         /** Returns the accessor that provides access to the complex content that is identified by the ProductProcessingStatus.
           *
           * @return The acecssor that provides access to the complex content that is identified by ProductProcessingStatus.
           */
         virtual const uci::type::ProductProcessingStatusType& getProductProcessingStatus() const
            throw(uci::base::UCIException) = 0;


         /** Returns the accessor that provides access to the complex content that is identified by the ProductProcessingStatus.
           *
           * @return The acecssor that provides access to the complex content that is identified by ProductProcessingStatus.
           */
         virtual uci::type::ProductProcessingStatusType& getProductProcessingStatus()
            throw(uci::base::UCIException) = 0;


         /** Sets the complex content that is identified by the ProductProcessingStatus to the contents of the complex content
           * that is accessed by the specified accessor.
           *
           * @param value The accessor that provides access to the sequence whose contents are to be used to set the contents of
           *      the sequence identified by ProductProcessingStatus
           */
         virtual void setProductProcessingStatus(const uci::type::ProductProcessingStatusType& value)
            throw(uci::base::UCIException) = 0;


         /** Returns whether the Element that is identified by ProductProcessingStatus exists (is enabled) or not.
           *
           * @return A boolean indicating whether the Element identified by ProductProcessingStatus is emabled or not
           */
         virtual bool hasProductProcessingStatus() const
            throw(uci::base::UCIException) = 0;


         /** Enables the Element that is identified by ProductProcessingStatus
           *
           * @param type = uci::type::accessorType::productProcessingStatusType This Accessor's accessor type
           */
         virtual void enableProductProcessingStatus(uci::base::accessorType::AccessorType type = uci::type::accessorType::productProcessingStatusType)
            throw(uci::base::UCIException) = 0;


         /** Clears (disabled) the Element that is identified by ProductProcessingStatus */
         virtual void clearProductProcessingStatus()
            throw(uci::base::UCIException) = 0;



      protected:

         /** The constructor [only available to derived classes]. */
         ProductProcessingTaskStatusMDT()
         { }

         /** The copy constructor [only available to derived classes]
           *
           * @param rhs The ProductProcessingTaskStatusMDT to copy from
           */
         ProductProcessingTaskStatusMDT(const ProductProcessingTaskStatusMDT& rhs)
         {
            (void)rhs;
         }

         /** The assignment operator. Sets the contents of this ProductProcessingTaskStatusMDT to the contents of the
           * ProductProcessingTaskStatusMDT on the right hand side (rhs) of the assignment operator.ProductProcessingTaskStatusMDT
           * [only available to derived classes]
           *
           * @param rhs The ProductProcessingTaskStatusMDT on the right hand side (rhs) of the assignment operator whose contents
           *      are used to set the contents of this uci::type::ProductProcessingTaskStatusMDT
           * @return A constant reference to this ProductProcessingTaskStatusMDT.
           */
         const ProductProcessingTaskStatusMDT& operator=(const ProductProcessingTaskStatusMDT& rhs)
         {
            (void)rhs;

            return *this;
         }


      }; // ProductProcessingTaskStatusMDT


   } // Namespace: type
} // Namespace: uci

#endif // Uci__Type__ProductProcessingTaskStatusMDT_h

