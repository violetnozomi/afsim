...
#ifndef P6DOFFUELTANK_H
#define P6DOFFUELTANK_H
...
// The P6DofFuelTank class serves as a fuel container for propulsion objects.
...
class P6DOF_EXPORT P6DofFuelTank : public P6DofObject
{
   friend class P6DofPropulsionSystem;

public:
   explicit P6DofFuelTank(P6DofScenario* aScenario);
   ...
   void SetPropulsionSystem(P6DofPropulsionSystem* aParent) { mPropulsionSystem = aParent; }
...
protected:
   ...
   double mMaxFlowRate_pps;     // Maximum output fuel rate to engines
   double mMaxFillRate_pps;     // Maximum fuel input rate during refueling
   double mMaxTransferRate_pps; // Maximum fuel transfer rate between tanks

   double mMaxQuantity_lbs;     // Maximum fuel quantity of tank
   double mCurrentQuantity_lbs; // Current amount of fuel in tank

   double mCurrentFuelFlow_pps;     // Current fuel flow rate to engines
   double mCurrentFillRate_pps;     // Current fuel fill rate
   double mCurrentTransferRate_pps; // Current fuel transfer rate

   double mTempCurrentFuelFlow_pps;     // Current fuel flow rate to engines
   double mTempCurrentFillRate_pps;     // Current fuel fill rate
   double mTempCurrentTransferRate_pps; // Current fuel transfer rate
