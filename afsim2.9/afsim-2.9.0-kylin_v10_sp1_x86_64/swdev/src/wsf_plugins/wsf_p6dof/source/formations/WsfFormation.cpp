...
WsfFormation::WsfFormation(WsfFormationManager& aManager, const std::string& aName)
   : UtScriptAccessible()
...
WsfFormation::WsfFormation(const WsfFormation&  aOther,
   ...
   , mAttached{aOther.mAttached}
{
   ...
   {
      WsfFormation* childPtr                 = mManager.DuplicateFormation(i.second, this, i.first);
      mChildren[childPtr->GetRelativeName()] = childPtr;
   }
}

WsfFormation::~WsfFormation()
{
...
//! immediately claimed by a WsfFormationManager; this method should not be called
//! directly, instead WsfFormationManager::Duplicate should be used so that a formation
...
std::unique_ptr<WsfFormation> WsfFormation::Duplicate(WsfFormationManager& aManager,
                                                      WsfFormation*        aParentPtr,
                                                      const std::string&   aQualifiedName,
                                                      const std::string&   aName) const
{
   return std::unique_ptr<WsfFormation>(new WsfFormation(*this, aManager, aParentPtr, aQualifiedName, aName));
}

bool WsfFormation::ProcessInput(UtInput& aInput)
{
   ...
   if (mManager.HasType(command))
   {
      ...
      WsfFormation* inputForm = mManager.CreateFormation(command, formName);
      if (!inputForm)
      {
...
int WsfFormation::GetNumChildFormations() const
{
...
WsfFormation* WsfFormation::GetChildFormation(const std::string& aName) const
{
...
std::vector<std::string> WsfFormation::GetDescendantFormationNames() const
{
...
int WsfFormation::GetNumMembers() const
{
   int retval{0};
...
RelativeManeuverKinematics WsfFormation::GetMemberKinematicState() const
{
   RelativeManeuverKinematics retval{};
   WsfP6DOF_Mover*            movPtr = GetMemberMover();
   if (movPtr)
   {
...
RelativeManeuverKinematics WsfFormation::GetMemberInitialKinematicState() const
{
   RelativeManeuverKinematics retval{};
   WsfP6DOF_Mover*            movPtr = GetMemberMover();
   if (movPtr)
...
void WsfFormation::SetMemberInitialKinematicState(RelativeManeuverKinematics& aState) const
{
   WsfP6DOF_Mover* mover = GetMemberMover();
   if (mover)
   {
...
WsfFormation* WsfFormation::GetFormationLeader()
{
   WsfFormation* retvalPtr{nullptr};
   if (IsLeaf())
   {
      retvalPtr = this;
   }
   ...
   {
      WsfFormation* leadForm = GetLeadSubFormation();
      if (leadForm)
      {
...
WsfFormation* WsfFormation::GetAttachedRoot()
{
   WsfFormation* retvalPtr{this};
   while (retvalPtr->IsAttached())
   {
      WsfFormation* next = retvalPtr->GetParentFormation();
      if (next)
      {
         retvalPtr = next;
      }
...
WsfFormation* WsfFormation::GetKeepStationRoot()
{
   WsfFormation* retval{nullptr};
   ...
   if (IsLeader())
   {
      ...
      WsfFormation* grandparentPtr = this;
      while (grandparentPtr->IsLeader() && grandparentPtr->IsAttached())
      {
         WsfFormation* nextPtr = grandparentPtr->GetParentFormation();
         if (nextPtr == nullptr)
         {
            break;
         }
   ...
   else
   {
      if (IsAttached())
      {
         WsfFormation* parentPtr = GetParentFormation();
...
bool WsfFormation::IsLeader() const
{
...
std::string WsfFormation::GetChildQualifiedName(const std::string& aChildName) const
{
   return mQualifiedName + "." + aChildName;
}

//! Return if this formation has the given as an ancestor.
bool WsfFormation::HasAncestor(const WsfFormation* aFormationPtr) const
{
   int                 counter{0};
   const WsfFormation* iter = this;
...
WsfFormationOffset WsfFormation::GetOffsetFrom(WsfFormation& aFormation) const
{
   WsfFormationOffset retval{};
   if (HasAncestor(&aFormation))
   {
      const WsfFormation* currPtr = this;
...
RelativeManeuverKinematics WsfFormation::GetOffsetKinematicsFrom(WsfFormation* aFormationPtr) const
{
   RelativeManeuverKinematics retval{};
   if (HasAncestor(aFormationPtr))
   {
      // walk up to ancestor saving off the offset along the way.
      std::vector<WsfFormationOffset> offsets{};
      const WsfFormation*             currPtr = this;
      ...
      WsfFormation* leaderPtr = aFormationPtr->GetFormationLeader();
      auto          state     = leaderPtr->GetMemberKinematicState();
      UtVec3d       omegaWCS  = state.GetTurnCircle().GetAngularVelocity();
      auto          func      = [&omegaWCS](const RelativeManeuverKinematics& aState, const WsfFormationOffset& aOffset)
      { return WsfFormation::ComputeOffsetKinematics(aState, aOffset, omegaWCS); };
...
WsfFormationManager& WsfFormation::GetManager() const
{
   return mManager;
}

//! Get the scenario in which the formation is a part.
WsfScenario& WsfFormation::GetScenario() const
{
...
bool WsfFormation::SetParentFormation(WsfFormation* aParentPtr, bool aRename)
{
   bool retval{false};
   ...
   else
   {
      ...
      if (simPtr != nullptr)
      {
         WsfFormationUpdateStationKeeping::Update(this, simPtr->GetSimTime());
      }
      retval = true;
   }
   return retval;
...
//!                        produce a conflict.
bool WsfFormation::SetRelativeName(const std::string& aRelativeName)
{
...
bool WsfFormation::AddChildFormation(WsfFormation* aChildPtr)
{
   bool success{false};
   if (aChildPtr)
   {
...
WsfFormation* WsfFormation::RemoveChildFormation(const std::string& aName, bool aRename)
{
   WsfFormation* retval{nullptr};
...
bool WsfFormation::SetLead(const std::string& aName)
{
   bool          retval{false};
   WsfFormation* childPtr = GetChildFormation(aName);
   if (childPtr)
   {
      mLeadChild = aName;

      WsfFormationOffset delta = childPtr->GetOffset();

      // Update all offsets of children by subtracting the saved offset.
      for (auto& i : mChildren)
      {
         WsfFormationOffset updatedOffset = i.second->GetOffset() -= delta;
      ...
      if (simPtr != nullptr)
      {
         // if there is a simulation, we are not intializing, so we will zero out the
         // overall offset.
         if (IsRoot())
         {
            mOffset = WsfFormationOffset();
         }
...
void WsfFormation::SetOffset(const WsfFormationOffset& aOffset)
{
   mOffset = aOffset;
}
...
bool WsfFormation::SetMemberName(const std::string& aPlatformName)
{
   bool          retval{false};
   WsfFormation* formPtr = GetManager().GetFormationFromMember(aPlatformName);
   if (formPtr != nullptr)
   {
...
WsfFormationCommand* WsfFormation::GetCurrentCommand() const
{
...
bool WsfFormation::ExecuteCommand(std::unique_ptr<WsfFormationCommand> aCommandPtr, double aSimTime)
{
   bool retval{false};
...
RelativeManeuverKinematics WsfFormation::ComputeOffsetKinematics(const RelativeManeuverKinematics& aKinematics,
...
void WsfFormation::UpdateQualifiedName()
{
...
bool WsfFormation::UpdateChildRelativeName(const std::string& aOldName, const std::string& aNewName)
{
   bool retval{false};
   if (aNewName.empty())
   {
...
void WsfFormation::AddDescendantNames(std::vector<std::string>& aNames) const
{
...
bool WsfFormation::SetMemberNameP(const std::string& aPlatformName)
{
   return false;
}
