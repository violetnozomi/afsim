...
bool wsf::six_dof::Terrain::ReadTerrainFile(const std::string& aFilename)
{
   ...
   {
      auto out = ut::log::error() << "Exception thrown while reading terrain file in Terrain::ReadTerrainFile.";
   ...
   {
      auto out = ut::log::error() << "Unable to open terrain file in Terrain::ReadTerrainFile.";
      out.AddNote() << "File: " << aFilename;
      return false;
   }
   catch (std::exception& e)
   {
      auto out = ut::log::error() << "Exception in terrain file in Terrain::ReadTerrainFile.";
      out.AddNote() << "File: " << aFilename;
...
bool wsf::six_dof::Terrain::ProcessInput(UtInput& aInput)
{
   ...
   if (aInput.GetCommand() == "terrain")
   {
      myCommand = true;
      UtInputBlock block(aInput, "end_terrain");
      ...
      while (block.ReadCommand(command))
      {
         ...
         {
            auto out = ut::log::error() << "Unrecognized command within Terrain::ProcessInput().";
...
bool wsf::six_dof::Terrain::ProcessRegion(UtInput& aInput)
{
   ...
   if (aInput.GetCommand() == "region")
   {
      ...
      while (block.ReadCommand(command))
      {
         ...
         {
            auto out = ut::log::error() << "Unrecognized command within Terrain::ProcessRegion().";
            out.AddNote() << "Command: " << command;
            out.AddNote() << "Location: " << aInput.GetLocation();
            throw UtInput::UnknownCommand(aInput);
         }
   ...
   {
      auto out = ut::log::error() << "Incomplete region definition in Terrain::ProcessRegion().";
...
bool wsf::six_dof::Terrain::ProcessLocation(UtInput& aInput, TerrainRegion& aRegion)
{
   ...
   if (aInput.GetCommand() == "location")
   {
      ...
      while (block.ReadCommand(command))
      {
         ...
         {
            auto out = ut::log::error() << "Unrecognized command within Terrain::ProcessLocation().";
            out.AddNote() << "Command: " << command;
            out.AddNote() << "Location: " << aInput.GetLocation();
            throw UtInput::UnknownCommand(aInput);
         }
   ...
   {
      auto out = ut::log::error() << "Incomplete region definition in Terrain::ProcessLocation().";
...
double wsf::six_dof::Terrain::HeightOfTerrain_m(double aLatitude, double aLongitude) const
{
   double alt_ft = 0.0;
   ...
   {
      float elev_m;
      mWsfTerrain->GetGeodeticElev(aLatitude, aLongitude, elev_m);

      return static_cast<double>(elev_m);
   }
}
                                                ...
                                                double&   aFrictionMultiplier) const
{
   ...
   // Check for specially defined terrain beneath us
   double alt_ft;
   if (HeightOfRegionLocation_ft(aLatitude, aLongitude, alt_ft))
   {
   ...
   else
   {
      float elev_m;
      mWsfTerrain->GetGeodeticElev(aLatitude, aLongitude, elev_m);
      // mWsfTerrain->GeodeticComputeNormal(aLatitude, aLongitude, aUpVectorNED.GetData());
      aUpVectorNED.Set(0.0, 0.0, -1.0);
      return static_cast<double>(elev_m);
   }
}

bool wsf::six_dof::Terrain::HeightOfRegionLocation_ft(double aLatitude, double aLongitude, double& aAlt_ft) const
{
...
bool wsf::six_dof::Terrain::WithinRegion(const TerrainRegion& aRegion, double aLatitude, double aLongitude) const
{
...
bool wsf::six_dof::Terrain::WithinLocation(const TerrainLocation& aLocation,
                                           double                 aLatitude,
                                           double                 aLongitude,
