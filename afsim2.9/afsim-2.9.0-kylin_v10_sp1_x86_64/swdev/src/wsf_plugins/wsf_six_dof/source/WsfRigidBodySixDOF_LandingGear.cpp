...
wsf::six_dof::RigidBodyLandingGear::RigidBodyLandingGear(const RigidBodyLandingGear& aSrc)
   : Object(aSrc)
   , mLastUpdateSimTime_nanosec(aSrc.mLastUpdateSimTime_nanosec)
   , mListOfGroundReactionPoints(aSrc.mListOfGroundReactionPoints)
...
{
   return new RigidBodyLandingGear(*this);
}
...
void wsf::six_dof::RigidBodyLandingGear::SetParentVehicle(Mover* aParentVehicle)
{
   ...
   {
      i->SetParentVehicle(GetParentVehicle());
   }
}
...
bool wsf::six_dof::RigidBodyLandingGear::ProcessInput(UtInput& aInput)
{
   ...
   if (command == "landing_gear")
   {
      ...
      while (block.ReadCommand())
      {
         ...
         {
            auto tempReactionPoint = ut::make_unique<RigidBodyGroundReactionPoint>();
            ...
            {
               mListOfGroundReactionPoints.push_back(std::move(tempReactionPoint));
            }
            else
            {
         ...
         {
            auto out = ut::log::error() << "Unrecognized command within RigidBodyLandingGear::ProcessInput().";
...
bool wsf::six_dof::RigidBodyLandingGear::Initialize(int64_t aSimTime_nanosec)
{
   ...
   if (GetParentVehicle())
   {
      if (!mNWSEnableControlName.empty())
      {
         ...
         mNWSEnableControlHandle =
            utils::CastToRigidBody(mParentVehicle)->GetFlightControls()->GetBooleanControlHandle(mNWSEnableControlName);
      }
...
void wsf::six_dof::RigidBodyLandingGear::ApplyExternalForce(double aForceMagnitude_lbs, double aAngle_rad)
{
...
void wsf::six_dof::RigidBodyLandingGear::Update(int64_t aSimTime_nanosec, UtVec3dX& aRequiredOpposingForceInSurfaceVecNED_lbs)
{
   if (utils::CastToRigidBody(mParentVehicle) == nullptr)
   {
      return;
   }
   ...
   {
      EnableNWS(utils::CastToRigidBody(mParentVehicle)->GetFlightControls()->GetControlBoolean(mNWSEnableControlHandle));
   }
   ...
   for (auto& point : mListOfGroundReactionPoints)
   {
      ...
      {
         double steeringAngle_deg = utils::CastToRigidBody(mParentVehicle)
                                       ->GetFlightControls()
                                       ->GetControlSurfaceAngle_deg(point->GetSteeringControlHandle());
         point->SetSteeringAngle_deg(steeringAngle_deg);
      }

      double brakingValue =
         utils::CastToRigidBody(mParentVehicle)->GetFlightControls()->GetControlValue(point->GetBrakingControlHandle());
   ...
   {
      mLastVehicleVelocityOnGround_mps = utils::CastToRigidBody(mParentVehicle)->GetVelocityNED_mps();
   }
   else
   {
...
void wsf::six_dof::RigidBodyLandingGear::GetForceAndMoments(UtVec3dX& aForce_lbs, UtVec3dX& aMoment_ftlbs) const
{
...
bool wsf::six_dof::RigidBodyLandingGear::VehicleIsAtRest() const
{
   ...
   UtVec3dX currentVel_mps = utils::CastToRigidBody(mParentVehicle)->GetVelocityNED_mps();
...
double wsf::six_dof::RigidBodyLandingGear::NoseGearDistForwardOfReference_ft()
{
   ...
   {
      RigidBodyGroundReactionPoint* bestNoseGear = nullptr;
...
double wsf::six_dof::RigidBodyLandingGear::RightMainGearRearwardOfReference_ft()
{
   ...
   {
      RigidBodyGroundReactionPoint* bestMainGear = nullptr;
      double                        bestDist_m   = -1.0;
...

{
   ...
   if ((turnRadiusSquared - mainBackSquared) < 0.0)
   {
      ...
      auto out = ut::log::debug() << "RigidBodyLandingGear::SteeringAngleForDesiredRadiusFt_deg():";
...
wsf::six_dof::RigidBodyGroundReactionPoint* wsf::six_dof::RigidBodyLandingGear::GetNoseGear()
{
...
wsf::six_dof::RigidBodyGroundReactionPoint* wsf::six_dof::RigidBodyLandingGear::GetRightMainGear()
{
...
wsf::six_dof::RigidBodyGroundReactionPoint* wsf::six_dof::RigidBodyLandingGear::GetLeftMainGear()
{
