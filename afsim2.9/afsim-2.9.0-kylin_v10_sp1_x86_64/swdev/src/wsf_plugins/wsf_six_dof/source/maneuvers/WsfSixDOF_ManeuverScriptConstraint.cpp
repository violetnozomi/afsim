...
void wsf::six_dof::ManeuverScriptConstraint::Assign(double aSimTime, Mover* aMoverPtr)
{
   mAssignTime = aSimTime;
   mMoverPtr   = aMoverPtr;
}

bool wsf::six_dof::ManeuverScriptConstraint::Satisfied(double aSimTime) const
{
   bool retval{true};
   if (mSatisfiedPtr != nullptr)
   {
      ...
      moverArg.SetPointer(new UtScriptRef(mMoverPtr, ctxPtr.GetClass("WsfSixDOF_Mover")));
...
double wsf::six_dof::ManeuverScriptConstraint::NextEvaluationTime(double aSimTime) const
{
   double retval{-1.0};
   if (mNextEvaluationTimePtr != nullptr)
   {
      ...
      UtScriptData moverArg;
      moverArg.SetPointer(new UtScriptRef(mMoverPtr, ctx.GetClass("WsfSixDOF_Mover")));
...
void wsf::six_dof::ManeuverScriptConstraint::SetSatisfiedScript(const std::string&   aScriptName,
...
//! \param aScriptName - the name of script.
//! \param aSimulation - the simulation in which this constraint in participating.
void wsf::six_dof::ManeuverScriptConstraint::SetNextEvaluationTimeScript(const std::string&   aScriptName,
                                                                         const WsfSimulation& aSimulation)
{
   if (!aScriptName.empty())
   {
      ...
      {
         // Validation isn't able to handle derived classes - disabling
         // bool ok = ctx.ValidateScript(mNextEvaluationTimePtr, "double", "double, Mover");
         // if (!ok)
         //{
