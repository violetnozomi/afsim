...
void wsf::six_dof::FormationUpdateStationKeeping::Update(Formation* aFormationPtr, double aSimTime, bool aJumpDetached)
{
   if (aFormationPtr->IsLeaf())
   {
      Formation* leadPtr  = aFormationPtr->GetKeepStationRoot()->GetFormationLeader();
      Mover*     moverPtr = aFormationPtr->GetMemberMover();
      if (moverPtr)
      {
         if (aFormationPtr == leadPtr)
         {
            // So we only want to do this if there is nothing going on currently.
            Maneuver* mvrPtr = moverPtr->GetCurrentManeuver();
   ...
   {
      auto func = [aJumpDetached, aSimTime](Formation* aSubPtr)
      {
