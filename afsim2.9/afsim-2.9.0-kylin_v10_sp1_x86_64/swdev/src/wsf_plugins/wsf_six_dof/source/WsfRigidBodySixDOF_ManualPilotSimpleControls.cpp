...
wsf::six_dof::RigidBodyManualPilotSimpleControls* wsf::six_dof::RigidBodyManualPilotSimpleControls::Clone() const
{
   return new RigidBodyManualPilotSimpleControls(*this);
}

wsf::six_dof::RigidBodyManualPilotSimpleControls::RigidBodyManualPilotSimpleControls(RigidBodyPilotObject* aPilotObject)
   : RigidBodyManualPilot(aPilotObject)
{
}

bool wsf::six_dof::RigidBodyManualPilotSimpleControls::ProcessInput(UtInput& aInput)
{
   bool               myCommand = true;
      ...
      while (block.ReadCommand())
      {
         ...
         else if (localCommand == "pitch_trim_factor")
         {
            ...
            if (value < 0)
            {
               ...
               auto out = ut::log::warning()
                          << "Negative pitch_trim_factor in RigidBodyManualPilotSimpleControls::ProcessInput(). Using "
         ...
         else if (localCommand == "roll_trim_factor")
         {
            ...
            if (value < 0)
            {
               value    = fabs(value);
               auto out = ut::log::warning()
                          << "Negative roll_trim_factor in RigidBodyManualPilotSimpleControls::ProcessInput(). Using "
                             "positive value instead.";
               out.AddNote() << "Using Value: " << value;
               out.AddNote() << "Location: " << aInput.GetLocation();
            }
         ...
         else if (localCommand == "yaw_trim_factor")
         {
            ...
            if (value < 0)
            {
               value    = fabs(value);
               auto out = ut::log::warning()
                          << "Negative yaw_trim_factor in RigidBodyManualPilotSimpleControls::ProcessInput(). Using "
                             "positive value instead.";
               out.AddNote() << "Using Value: " << value;
               out.AddNote() << "Location: " << aInput.GetLocation();
            }
         ...
         {
            auto out =
               ut::log::error() << "Unrecognized command within RigidBodyManualPilotSimpleControls::ProcessInput().";
...
bool wsf::six_dof::RigidBodyManualPilotSimpleControls::Initialize(int64_t aSimTime_nanosec)
{
