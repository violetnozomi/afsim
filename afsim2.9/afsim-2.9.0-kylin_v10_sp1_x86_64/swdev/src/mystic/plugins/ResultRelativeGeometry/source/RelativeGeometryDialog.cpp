...
void RelativeGeometry::Dialog::GuiUpdate()
{
   ...
   {
      delete mRangeRateItem;
      mRangeRateItem = nullptr;
   }
}
...
void RelativeGeometry::Dialog::PlotItem(QTreeWidgetItem* aItemPtr) const
{
   ...
   auto updaterPtr = ut::make_unique<PlotUpdater>("Time", aItemPtr->text(0), fromName, toName);
   if (updaterPtr != nullptr)
   {
      rv::PlottingWidget* plotWidget =
         vaEnv.GetFactory()->CreateDockWidget<rv::PlottingWidget>("PlottingWidget",
