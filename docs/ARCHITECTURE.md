...
当前`0.6.0`实现第一条完整纵向路径：Warlock仿真接口监听通信事件并扫描全局通信图，生成
网络、端点、链路、位置、距离和消息值对象。GUI线程消费快照，文件工作线程异步输出JSONL
和CSV。纯C++ `AssessmentEvaluator`只读取一份不可变快照，在GUI任务提交时严格分离当前
启用图和参数化候选图，计算主路由、边不重合备选路由及硬约束结果；评估结果通过同一
文件线程写出，并由中央态势图叠加显示。
...
0.6的典型图规模很小，用户点击后在GUI线程执行确定性路径搜索。进入200节点压力场景
