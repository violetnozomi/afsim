...
- 坐标模型：新增`NrmTacticalViewport`，单独维护50%–500%、每次25%的内容到视口仿射
  ...
  “适配全图”恢复100%和零平移。
- 绘制分层：仅网格、节点、链路、网关、算法逐跳路径和随节点标签进入缩放/平移变换；
  标题、摘要、选择提示、底部图例和节点详情卡使用固定屏幕坐标。右侧资源面板完全不参与
  视口变换。
- 交互：左键按下后，移动超过Qt拖动阈值才开始平移；未达到阈值则按单击处理。节点命中先将
  屏幕坐标逆变换为内容坐标，继续支持普通详情查看和资源测评源/目的节点选择。滚轮产生的
  百分比会回传右侧缩放栏，按钮和滚轮保持同一状态。
