...
_对照《AFSIM网络资源管理器技术报告与实施方案》· 更新日期：2026-08-20_
...
| 综合通信协同场景 | 37节点、53端点、108条有向链路；含12个双归属网关，无敌方、武器和交战元素 | 0.12.0工作区 |
| 跨域网关运行时 | 六组域对、12个方向能力、12条直达路由、2条双网关级联；策略/场景/运行时已验证 | PRE_ACCEPTANCE |
...
| 现代化可视化 | 深色卡片、统一控件、平台去重、多网徽点、标签避碰、80%–160%同步缩放和中文优先显示已验证 | VNC实机100%→110%及放大后节点点击 |
| 合同30行指标软件映射 | 30/30已实现，26类唯一能力，PRE_ACCEPTANCE | `docs/合同30项指标实现矩阵.md` |
| 合同补缺闭环 | 已实现，40项固定C++测试通过 | 频点/协议资源、区域/姿态/干扰、规划协同、需求反馈、导航精度、恢复、部署、甲方入口、跨域网关及界面缩放证据 |
...
| 路由 | 当前/候选视角、有界 K 路径、一条备路、显式跨网网关路由和逐跳审计 | 本地设备路由表和甲方正式跨域策略映射 |
...
网络资源管理与评估链保持观察/建议模式，不自动建链、改频或改路由。新增网关处理器仅按
场景中预先声明的显式路由转发带网关元数据的消息，不修改实时网络配置。
