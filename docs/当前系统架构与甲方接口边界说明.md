...
- `NavigationSample.platformId`是导航状态主键；Customer覆盖层只保存Customer真实上报的
  平台，同平台覆盖AFSIM，其他平台继续使用最新AFSIM导航。
- 环境按地形、气象、天象和干扰四个子域覆盖；Customer未提供的子域保留AFSIM状态。
- 同一`runId`内，资源全量域、导航平台和环境子域分别更新、统一合并，任何更新都不会
  清空其他已接受状态。
