...
2. 不便直接链接时，使用项目提供的精简 JSON Schema V1 交换文件或消息。
...
flowchart LR
    ...
    subgraph boundary ["需要双方对齐的接口边界"]
        cpp_adapter["同进程C++适配器"]
        json_adapter["JSON Schema V1适配器"]
...
| 外部适配 | `ContractInterfaceAdapter`、JSON Codec | 把甲方数据转换为公共值对象 | **是** |
...
| 参数剖面 | `NetworkProfileRepository` | 保存四网默认频率、容量、范围和业务类型 | 甲方有真实参数时可覆盖 |
...
| 资源规划 | `NetworkPlanRepository`及规划服务 | 规划加载、编辑、校验、推演、存储和本地分发包 | **规划输入/结果需要对齐** |
| 资源管理 | 需求Repository、匹配服务、建议引擎 | 需求匹配、差距分析和六类规划建议 | **需求输入/结果需要对齐** |
...
双方接口控制在八类业务消息内，不要求甲方对齐插件内部的30项类或算法。
...
甲方只需要确认输入字段能否提供、输出字段能否接收，以及在其AFSIM目标树中采用同进程
C++还是JSON方式接入。
